...
def trouve_pos_exact(xy, name_img = "image_exemple.png", name_out = " ", h = 25, w = 25, aire_min = 16,  aire_max = 30, ecritur = False):
    ...
    for coord in xy:
        ...
        #trouve contour
        ret, thresh = cv.threshold(img_gray1, 132, 255, cv.THRESH_BINARY) #2eme paramètre a retravailler pour ajuster contraste
        ...
        for cnt in contours:
            ...
            #print("aire = ",A)
            #print("radius = ",radius)
            if(A<aire_max and A>aire_min and radius<16 and radius>2) :
                approx = cv.approxPolyDP(cnt,0.01*cv.arcLength(cnt,True),True)
                #cv.drawContours(cropped,[cnt],0,255,-1)
                #cv.imwrite("cnt_" + str(x) + "_" + str(y) + "_aire_" + str(A) + "_radius_" +str(radius) + ".png", cropped)
